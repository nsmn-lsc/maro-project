...
export default function ColegiadosPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if ((session.nivel ?? 0) < 3) {
        // Nivel 2 (regional) → su módulo; nivel 1 → dashboard unidad
  ...
  const pendientes = useMemo(
    () => registros.filter((r) => (Number(r.acciones_total) || 0) === 0),
    [registros]
  );
    ...
    <main className="min-h-screen relative text-slate-100" style={bgStyle}>
      ...
      <div className="relative max-w-7xl mx-auto space-y-6 p-6 lg:p-10">
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <Card title="Pendientes de acciones" value={String(pendientes.length)} />
          <Card title="Con acciones definidas" value={String(configurados.length)} />
        </section>
        ...
        <section className="bg-slate-900/60 border border-slate-700 rounded-2xl p-4">
          {loading ? (
            ...
            <div className="space-y-3">
              <p className="text-slate-300">No hay casos pendientes de captura de acciones.</p>
              <Link href="/colegiados/planes" className="inline-flex text-sm px-3 py-1.5 rounded-full border border-emerald-500/50 text-emerald-200 hover:border-emerald-300">
                Ver casos con acciones ya establecidas
              </Link>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-800">
                  {pendientes.map((row) => (
                    <tr key={row.consulta_id} className="hover:bg-slate-800/40">
                      ...
                      <td className="py-2 pr-4">
                        <span className="inline-flex items-center rounded-md px-2 py-1 text-xs font-semibold bg-amber-500/15 text-amber-200 border border-amber-500/30">
                          Sin acciones
                        </span>
                      </td>
                      <td className="py-2 pr-0 text-right">
                        <Link
                          href={`/colegiados/${row.consulta_id}`}
